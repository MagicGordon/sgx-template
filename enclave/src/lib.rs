...
pub extern "C" fn ecall_init() -> sgx_status_t {
    ...
    let (attn_report, sig, cert) = match create_attestation_report(&pub_k, sgx_quote_sign_type_t::SGX_UNLINKABLE_SIGNATURE) {
        Ok(r) => r,
        Err(e) => {
    ...
    match cert::verify_mra_cert(&cert_der) {
        Ok(()) => (),
        Err(sgx_status_t::SGX_ERROR_UPDATE_NEEDED) => (),
        Err(e) => {
            println!("verify_mra_cert failed: {:?}", e);
            return e;
        }
    }
...
pub extern "C" fn ecall_action(
    ...
    let input_value = input.get("operation").unwrap().clone();
    println!("enclave receive action {}", input_value);

    let output_json = json!({"result": input_value});
